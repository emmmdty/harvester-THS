...
async function main() {
  if (resolveHeadless()) {
    throw new Error("当前是无头模式，无法交互登录。请在有桌面环境的机器上运行登录，或挂载已有的 .bilibili-profile 登录目录。");
  }
  ...
  await page.goto(BILIBILI_URL, { waitUntil: "domcontentloaded" });
  await page.bringToFront().catch(() => {});
  activateChromiumWindow();
  console.log("B站登录浏览器已打开。登录完成后关闭这个浏览器窗口，再回到面板开始爬取。");
